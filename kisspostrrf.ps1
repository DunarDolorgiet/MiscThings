<#
.SYNOPSIS
Postprocessing for KISSlicer generated gcode for RRF based boards
...
Changes RRF acceleration based on path type. Definded via "solid_path_accel_mm_per_s_per_s = number",.. in gcode.
        ...
        switch ($match.Groups[1].Value) {
            ...
            Perimeter {
                if ($global:currentAccl -ne $perimeterPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $perimeterPathAccl, $match.Groups[3].Value);
            ...
            Loop {
                if ($global:currentAccl -ne $loopPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $loopPathAccl, $match.Groups[3].Value);
            ...
            Travel {
                if ($global:currentAccl -ne $travelPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $travelPathAccl, $match.Groups[3].Value);
            ...
            Solid {
                if ($global:currentAccl -ne $solidPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $solidPathAccl, $match.Groups[3].Value);
            ...
            'Stacked Sparse Infill' {
                if ($global:currentAccl -ne $infillPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $infillPathAccl,$match.Groups[3].Value);
            ...
            Crown {
                if ($global:currentAccl -ne $crownPathAccl) {
                    $match.Groups[0].Value -replace $match.Groups[3].Value, ("M201 X{0} Y{0}`n{1}" -f $crownPathAccl,$match.Groups[3].Value);
            ...
            default {
                if ($global:currentAccl -ne $defaultPathAccl) {
                    $match.Groups[0] -replace $match.Groups[3], ("M201 X{0} Y{0}`n{1}" -f $defaultPathAccl,$match.Groups[3]);
...
$eAxisMatchEval = {
    param($match)
    try {
        $ret = $match.Groups[0].Value -replace ("E{0}" -f $match.Groups[1]), (("E{0}" -f ($match.Groups[1].Value/1 * $crownExtrusionModifier)).ToString((New-Object Globalization.CultureInfo "")));
        $ret;
    } catch {
        $match.Groups[0].Value;
    }
